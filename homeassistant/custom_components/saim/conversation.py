...
async def async_setup_entry(
    ...
    """Set up the SAiM conversation entity."""
    async_add_entities([SaimConversationEntity(entry)])


class SaimConversationEntity(conversation.ConversationEntity):
    ...
    async def _try_home_assistant(
        ...
        except Exception:  # noqa: BLE001 - HA's agent must never take SAiM down
    ...
    async def _ask_neuro(
        ...
        if not ok:
            _LOGGER.warning("SAiM could not answer after %.1fs: %s", elapsed, problem)
        ...
        _LOGGER.debug("SAiM answered in %.1fs", elapsed)
    ...
    async def _post(
        ...
        try:
            ...
            _LOGGER.warning("SAiM could not reach NEURO at %s: %s", url, err)
