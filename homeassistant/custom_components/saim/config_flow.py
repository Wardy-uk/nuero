...
class SaimConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle a config flow for SAiM."""
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            return self.async_create_entry(title="SAiM", data=user_input)
    ...
    def async_get_options_flow(entry: ConfigEntry) -> OptionsFlow:
        return SaimOptionsFlow()


class SaimOptionsFlow(OptionsFlow):
