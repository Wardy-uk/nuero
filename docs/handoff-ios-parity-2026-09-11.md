# iOS SAiM — parity gaps against the web SAiM (11 Sep 2026)

For the session working on `Wardy-uk/nuero-ios`. Found by a code-level audit of the
iOS repo at `c5d590d` against the web SAiM (`saim/app` + `saim/shared-ui`, which
...
now live in `saim/shared-ui/fieldDrive.mjs`, mirroring `FieldDrive.swift`, pinned
...
| "Show me everything" | `reveal` utterance | `SaimState.say` sets `showEverything`, which nothing reads — the escape hatch is a no-op. |
...
| Muted prompts | `GET /api/attention/muted`, `DELETE /api/attention/muted/:kind` | NEW 11 Sep. Before this the only way back was asking SAiM in the standup. |
...
- `SaimState.say` → `session` intent swallows errors with `try?`.
- `pendingPlan` (notification routing) is stored and never consumed.
- `SaimState.complete(_:)`, `snoozeNudge`, `FreshnessBar`, `SaimScreen` are unused.
...
- `SaimWatchComplication/` has no Xcode target, so it is never built; the watch app cannot auto-pair because the phone never sends the PIN.
