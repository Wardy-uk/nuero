// SAiM runtime backend — WS0-WP1 foundation.
//
...
//      process on one port (clean for PM2 + "one SAiM").
//
...
// Minimal .env loader (no dependency) — loads saim/backend/.env into process.env so the
...
// Pre-rename SARA_* variables (SAIM_PORT, SAIM_HA_TOKEN, …) are still spelled
// the old way in the .env deployed on the Pi — .env files are gitignored, so
// the rename never touched them. Carried over HERE: after the loader above has
// populated process.env, and before any route module reads it.
...
const app = express();
const PORT = process.env.SAIM_PORT || 3005;
...
// The capture bridge. SAiM's frontend has always POSTed here; nothing was mounted, so
...
// this catches what is left and forwards it to NEURO with SAiM's credential, so
...
app.listen(PORT, () => {
  console.log(`[SAiM ${RUNTIME_LABEL}] backend listening on http://0.0.0.0:${PORT}`);
  // Validate the NEURO dependency FIRST and say so out loud. SAiM is a manifestation
  ...
  // where a PIN gets entered, so a SAiM that refuses to boot cannot be repaired from
  ...
  // SAIM_GREET_SPEAKERS names the rooms.
  ...
  // Off (and says so) unless SAIM_CHARGE_KEEPERS names the rooms.
