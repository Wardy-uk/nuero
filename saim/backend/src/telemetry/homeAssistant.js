// SAiM Home Assistant telemetry bridge — v0 (WS3-WP1).
//
...
const SIGNAL_SLOTS = {
  location: 'SAIM_HA_LOCATION_ENTITY', // e.g. person.nick / device_tracker.nick_phone
  presence: 'SAIM_HA_PRESENCE_ENTITY', // e.g. binary_sensor.office_occupancy / person.nick
  environment: 'SAIM_HA_ENV_ENTITY', // e.g. sensor.office_temperature
  // Proximity drives SAiM's "walked away" auto-lock. It is deliberately source-agnostic
  ...
  // threshold (metres) is read from SAIM_HA_PROXIMITY_AWAY_M when the state is numeric.
  proximity: 'SAIM_HA_PROXIMITY_ENTITY',
};
...
function config() {
  ...
  return {
    baseUrl: (process.env.SAIM_HA_BASE_URL || '').replace(/\/+$/, ''), // e.g. http://homeassistant.local:8123
    token: process.env.SAIM_HA_TOKEN || null, // HA long-lived access token
    entities,
    pollMs: Number(process.env.SAIM_HA_POLL_MS) || 30000,
    timeoutMs: Number(process.env.SAIM_HA_TIMEOUT_MS) || 4000,
    // Metres beyond which a numeric (BLE distance) proximity signal counts as "away".
    proximityAwayM: Number(process.env.SAIM_HA_PROXIMITY_AWAY_M) || 8,
  };
}
...
// --- Normalisation (pure) ---------------------------------------------------
// Map a raw HA entity state object into a SAiM telemetry signal. Kept pure and
...
// Proximity → SAiM's "are you here?" signal for auto-lock. Source-agnostic by design:
...
function start() {
  ...
  const cfg = config();
  if (!isConfigured(cfg)) {
    console.log('[SAiM HA] telemetry bridge idle — not configured. Screens use fallback (location from seed).');
    return false;
  }
  refresh().catch(() => {});
  timer = setInterval(() => refresh().catch(() => {}), cfg.pollMs);
  if (timer.unref) timer.unref(); // never hold the process open just for polling
  console.log(`[SAiM HA] telemetry polling every ${cfg.pollMs}ms against ${cfg.baseUrl}`);
  return true;
}
