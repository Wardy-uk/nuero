...
const ENDPOINTS = {
  // Pending AI approvals — the one AUTH-gated call (saim service-account JWT in
...
function config(env = process.env) {
  return {
    ...
    pollMs: Number(env.SAIM_NOVA_POLL_MS) || 60000,
    timeoutMs: Number(env.SAIM_NOVA_TIMEOUT_MS) || 6000,
  };
}
...
function start() {
  ...
  const cfg = config();
  if (!isConfigured(cfg)) {
    console.log('[SAiM NOVA] snapshot bridge idle — no base URL.');
    return false;
  }
  refresh().catch(() => {});
  ...
  console.log(`[SAiM NOVA] snapshot polling every ${cfg.pollMs}ms against ${cfg.baseUrl} ${auth}`);
  return true;
}
