...
// seconds on a SAiM with no NEURO configured at all. See neuroConfig.js.
function config(env = process.env) {
  return {
    ...
    timeoutMs: Number(env.SAIM_NEURO_TIMEOUT_MS) || 6000,
    pollMs: Number(env.SAIM_GRAPH_POLL_MS) || 90000,
    seeds: Number(env.SAIM_GRAPH_SEEDS) || 4,
  };
}
...
function start() {
  ...
  if (!neuroConfig.readiness().ready) {
    console.log('[SAiM NEURO] vault cognition-graph idle — NEURO connection is not configured.');
    return false;
  }
  refresh().catch(() => {});
  timer = setInterval(() => refresh().catch(() => {}), cfg.pollMs);
  if (timer.unref) timer.unref();
  console.log(`[SAiM NEURO] vault cognition-graph polling every ${cfg.pollMs}ms against ${cfg.baseUrl}`);
  return true;
}
