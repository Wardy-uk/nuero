// SAiM State Engine — v1 contract (WS1-WP1).
//
...
function validate(model) {
  ...
  if (!isObject(model.saim) || typeof model.saim.name !== 'string' || typeof model.saim.status !== 'string') {
    errors.push('saim must be an object with name and status strings');
  }
  ...
  // Current location and confidence are part of SAiM's situational state (WS1
  ...
  // Telemetry is the Home Assistant signal block folded into SAiM's situational state
