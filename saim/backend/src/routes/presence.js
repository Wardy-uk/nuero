// GET /api/presence — compact "are you here?" signal for the SAiM auto-lock.
//
// The SAiM frontend lock logic polls this (small payload) rather than the full
// /api/state, so the wall display can cheaply ask "should I lock?" on a tight interval.
//
// SAIM_PRESENCE_SOURCE picks WHICH question this route answers. The two modes are
...
//                       present, so SAiM stays unlocked while you are anywhere in the
...
//   true  -> SAiM may auto-lock (you appear to have left)
...
const SENSOR_TOKEN = (process.env.SAIM_SENSOR_TOKEN || '').trim();
...
//   SAIM_OFFSITE_ROOMS=work-office
function isOffsite(room) {
  return String(process.env.SAIM_OFFSITE_ROOMS || '')
    .split(',')
    .map(s => s.trim())
    .filter(Boolean)
    .includes(room);
 ...
 * screen — and a desk twenty miles away answered yes. The study screen showed SAiM,
...
// restarting SAiM alone — no code change, and nothing else in the process caches it.
//
// ⚠ The DEFAULT is 'home' (Nick's call, 30 Aug 2026) — house-level geolocation, not the
// Apple Watch. Set SAIM_PRESENCE_SOURCE=watch to go back to desk-level proximity. The
...
function presenceSource() {
  return (process.env.SAIM_PRESENCE_SOURCE || 'home').toLowerCase() === 'watch' ? 'watch' : 'home';
}
