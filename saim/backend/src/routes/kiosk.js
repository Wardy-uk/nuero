...
// SAiM kiosk (Chromium --kiosk blocks window.close() for its main tab). The frontend
// Exit button calls this; we kill only the kiosk browser. The desktop launcher
// (scripts/start-saim.sh) runs Chromium in the foreground with an EXIT trap that
// restores the Pi taskbar, so killing Chromium lets the launcher exit cleanly and the
// user lands back on the Pi desktop. This backend (PM2 saim-backend) keeps running, so
...
router.post('/exit', (_req, res) => {
  if (!browserRunsHere()) {
    return res.json({
      ...
      error: 'This screen’s browser runs on another machine, so SAiM can’t close it from here.',
    });
  }
