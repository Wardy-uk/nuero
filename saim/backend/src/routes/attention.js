 ...
 *
 * ⚠ This is the seam where `saim/backend` last grew a second brain.
...
function fail(res, status, reason, detail) {
  ...
  // cannot tell "NEURO is down" from "SAiM is misconfigured". The reason is the
...
function createRouter(options = {}) {
  ...
  const timeoutMs = Number(env.SAIM_NEURO_TIMEOUT_MS) || 5000;
