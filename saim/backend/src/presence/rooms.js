...
//   full    the watch is in THIS room  -> SAiM, everything, on screen
...
// the watch STRAIGHT INTO A LOCK, which is why it "incorrectly locked SAiM": a
...
// it the screen alternates between full SAiM and the clock while he sits still -
...
const SLEEP_ROOM = process.env.SAIM_SLEEP_ROOM || 'bedroom';
const SLEEP_LOCK_MS = (Number(process.env.SAIM_SLEEP_LOCK_MINUTES) || 30) * 60_000;
 ...
 *
 * Nick, 12 Sep 2026, putting a SAiM screen on his desk at work: "amend SAiM's
 ...
 * made-up "back soon" on a screen his colleagues walk past is a promise SAiM has no
...
function offsiteDisplayState(thisRoom, arbitration, { mobile = [], meeting = null } = {}) {
  ...
  // ⚠ THE LAPTOP ONLY EVER SOFTENS THE CLOCK, never shows SAiM. It knows he is
  ...
  if (!own || !own.readable) {
    ...
    // SAiM: this screen is in a room other people walk through, so an unreadable
...
function displayState(thisRoom, arbitration, home, inferred = null, sustained = null, { offsite = false } = {}) {
  ...
  // the watch, SAiM is visible; if not, just the clock." Arbitration survives
