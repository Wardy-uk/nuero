...
Build the next two SAiM views on top of the converged many-views architecture while keeping all data ownership in shared state.
...
Build against the converged `nuero` SAiM runtime and the existing many-views architecture under `saim/frontend/src/`.
...
4. One factual build-status report in `saim/attractor/build_status/WS2A-WP1.md`.
