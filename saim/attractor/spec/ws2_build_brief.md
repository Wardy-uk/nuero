...
Build the first SAiM screen, Mission Control v0, while establishing a many-views UI architecture over the existing shared State Engine contract.

## Required Behavioural Outcome

Deliver a runtime where:

1. SAiM can be launched from a Pi desktop icon or via documented desktop-launcher installation steps
...
Build against the converged `nuero` SAiM runtime already present under `saim/`.
...
5. One factual build-status report in `saim/attractor/build_status/WS2-WP1.md`.
