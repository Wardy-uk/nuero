...
Add the first bounded context-inference layer so SAiM can answer "what does it think Nick is doing right now?" and "which view would it recommend?" using the converged state and telemetry seam.
...
4. If inference inputs are missing or contradictory, SAiM surfaces that honestly rather than pretending certainty.
