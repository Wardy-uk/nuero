...
Connect a bounded set of Home Assistant telemetry signals into SAiM's one shared
...
or automatic view selection were added. All work is in `saim/backend/`.

## What was added or changed

```
saim/backend/src/telemetry/
...
- **Configuration & connection path** — `homeAssistant.js` reads `SAIM_HA_BASE_URL`,
  `SAIM_HA_TOKEN`, and three bounded entity ids (`SAIM_HA_LOCATION_ENTITY`,
  `SAIM_HA_PRESENCE_ENTITY`, `SAIM_HA_ENV_ENTITY`) from env. It polls each entity via
...
- **Fallback path, live backend** (`node server.js`, port 3199, HA **not** configured):
  - Boot log: `[SAiM HA] telemetry bridge idle — not configured. Screens use fallback`.
...
- **Live ingestion path, end-to-end** (SAiM on port 3200 pointed at a mock HA serving
...
- HA auth is a long-lived token in env; the SAiM backend itself still has no PIN/token
