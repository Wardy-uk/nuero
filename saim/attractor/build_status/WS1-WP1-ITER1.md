...
Close the missing **location** and **confidence** signal so SAiM's situational state
(state + location + confidence) is exposed consistently on both runtime surfaces and
in the UI. All changes are inside `saim/`. The `/api` runtime path, server boot, route
...
saim/frontend/src/App.jsx   + location and confidence rows in the runtime metadata,
                    read from the same /api/state payload.
saim/frontend/src/App.css   + .saim__confidence--high/moderate/low colour cues.
saim/README.md              State Engine v1 section notes location + confidence.
```

## What "location" and "confidence" mean here

- **Location** — SAiM's current situational location. A **seeded** input in WS1
...
- **Production single-process path** (`SAIM_PORT` set, serving built `dist/`):
