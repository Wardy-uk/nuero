...
saim/backend/package.json   + "test": "node --test"
saim/frontend/src/App.jsx   updated to render the v1 model (briefing + per-domain
                          summary); seed banner replaces the WS0 placeholder flag;
                          WS0 duplicated-footer nit fixed
saim/frontend/src/App.css   + .saim__briefing / .saim__domains styles
saim/README.md              State Engine v1 section + updated limitations
```
...
- **Enforced shape:** `validate(model)` checks the root keys, the `saim` block, a
...
- **One engine, real work:** `buildModel()` assembles the single model from named
  providers and **derives** SAiM's briefing line from the assembled data (not a
...
- Root `dataSource: 'seed'`; every domain `source: 'seed'`; `saim.note` states the
...
- **Live backend** (`node server.js`, `SAIM_PORT=3066`):
...
- **Production single-process path** (`SAIM_PORT=3067`): `GET /` -> `200 text/html`,
