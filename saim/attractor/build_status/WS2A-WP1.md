...
Add the next two real SAiM views — **Executive Dashboard v0** and **Presence Mode
...
All changes are inside `saim/frontend/src/`. The backend (`server.js`,
...
saim/frontend/src/
  state/
    ...
    saimState.jsx            exposes `presentation: SHARED_PRESENTATION` (flattened from the
...
Both screens call `useSaimState()` and render only from it — the WS1 engine model, the
...
| Element | Source (all via `useSaimState()`) |
|---|---|
| Header state / seed pill | `model.saim.status`, `model.dataSource` |
...
- **Mission Control still works:** loads as the default view and renders all nine
  elements from shared state (SAiM / ONLINE / SEED DATA, clock, location, confidence,
...
4. **Both read shared state, owning no authoritative state** — both are pure
   `useSaimState()` consumers; the only "new" UI fields are the shared placeholder
