...
under `saim/` and are listed by `rg --files saim`. The likely cause of the
discrepancy: the entire `saim/` tree is **untracked in git** — `git status`
shows `?? saim/` and `git ls-files saim` returns nothing. A verification that
relied on git tracking rather than the working tree would correctly report
"missing", because nothing under `saim/` has been committed yet. The files exist
and are reviewable now; they are simply not yet staged/committed. Committing
`saim/` (its `.gitignore` already excludes `node_modules/`, `dist/`, `.env`)
makes the artefacts visible to any git-based check.

## What was added or changed

A new `saim/` runtime scaffold (greenfield — no `saim/` existed before). Nothing
...
saim/
  ...
  backend/                          Express runtime backend (CommonJS)
    ...
    .env.example                    SAIM_PORT (default 3005)
...
- **Production / Pi 5 (auto-start on boot):** the Pi 5 already runs PM2 under
  ...
  the frontend, starts the `saim-backend` PM2 process, and runs `pm2 save` so it
...
- Backend booted (`node server.js`, `SAIM_PORT=3055`):
  - `GET /api/health` → `{"status":"ok","saim":"online","runtime":"WS0-WP1","placeholder":true,...}`
  - `GET /api/state` → placeholder state object, `placeholder:true`, `runtime:"WS0-WP1"`,
    empty `domains:{}`, with `startedAt`/`servedAt` timestamps.
  - `GET /` → HTTP 200, serves built `index.html` (`<title>SAiM</title>`).
...
and the `saim/` scaffold is materially present and reviewable.

## Assumptions and local dependencies

- Target is the Pi 5 (`100.100.28.58`, `/mnt/data/nuero/saim`), Node v20.20.2
...
- State is an **in-memory placeholder** (`stateEngine.js`), flagged
  ...
  holds no real data.
- No auth on the SAiM backend yet (NEURO's PIN/token middleware not ported).
...
- One SAiM, one shared state model: all state flows through the single
