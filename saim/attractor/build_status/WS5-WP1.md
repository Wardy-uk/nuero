...
Add the first bounded context-inference layer over the converged SAiM state and the
Home Assistant telemetry seam. SAiM can now answer two questions from the one shared
...
- **Frontend, live in browser** (Vite dev on 5176 proxying `/api` to the backend on 3005):
  - The advisory strip renders: "SAiM THINKS — You look like you're firefighting — 2 tickets
