...
Build the first usable SAiM screen — **Mission Control v0** — on top of a
...
All changes are inside `saim/`. The backend (`server.js`, `src/state/*`,
...
saim/frontend/
  ...
  src/state/                      NEW  the shared UI state/context layer
    saimState.jsx                 SaimStateProvider + useSaimState() — reads /api/state,
                                  holds a shared live clock and the current-view selection
    views.js                      SaimView registry + DEFAULT_VIEW (the "view type")
...
saim/scripts/start-saim.sh        NEW  Pi desktop display launcher (opens UI full-screen)
saim/desktop/SAiM.desktop         NEW  XDG desktop entry (the clickable icon)
saim/desktop/saim.svg             NEW  launcher icon
saim/desktop/README.md            NEW  install + usage instructions
saim/README.md                    + "Mission Control & the view system (WS2 scope)" section,
                                  updated layout tree
```
...
definition" is expressed as the plain-JS `SAIM_VIEWS` / `VIEW_REGISTRY` in
`views.js`.
...
- **In-app shared layer:** `frontend/src/state/saimState.jsx` — a single
  `SaimStateProvider` (consumed via `useSaimState()`) that assembles the engine model
...
`screens/mission-control/MissionControl.jsx` calls `useSaimState()` and renders only
from it. It owns no data — it formats and orders shared state into the screen:

| Mission Control element | Source (all via `useSaimState()`) |
|---|---|
| SAiM header / current state | `model.saim.status` (+ `dataSource` seed pill) |
...
- **`desktop/SAiM.desktop`** — an XDG desktop entry (icon `desktop/saim.svg`) whose
  `Exec=` runs `scripts/start-saim.sh`. Assumes the Pi 5 deployment path
  `/mnt/data/nuero/saim`.
- **`scripts/start-saim.sh`** — the *display* launcher. It health-checks
...
- **`desktop/README.md`** — install/usage instructions (`chmod +x`, copy to
  `~/Desktop` and `~/.local/share/applications`, `SAIM_URL` override, leaving kiosk
  mode). The `saim/README.md` "Launch on the Pi desktop" section links to it.
...
- **Rendered UI** (accessibility snapshot of the running page): all nine required
  Mission Control elements present and populated from shared state — SAiM header +
...
- **One state, many views (principle 7):** every screen reads the one shared
  `useSaimState()` value; the view system (`views.js` + `ViewRouter` + `ViewSwitcher`)
