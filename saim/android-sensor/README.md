# SAiM room sensor — Android

The study's room sensor, running on the old Galaxy Tab A 10.1 (2016, SM-T585, Android
8.1) that is also the study's SAiM screen. It is the Android twin of
[`saim/sensor/saim-room-sensor.py`](../sensor/saim-room-sensor.py): same IRK match,
...
- **Android 8.1+ delivers nothing to an unfiltered scan while the screen is off.**
  ...
  Pis. Screen off (SAiM's locked state): narrowed to Apple adverts, and the reading
...
1. **Download** on the tablet, in Chrome:
   `https://github.com/Wardy-uk/nuero/releases/download/android-sensor-latest/saim-sensor.apk`
   Allow Chrome to install unknown apps when prompted.
2. **Open SAiM Sensor** and fill in:
   - Room: `study`
   ...
   - Watch IRK: the 32 hex characters from `/etc/saim-watch.env` on pi5
     (`sudo cat /etc/saim-watch.env`). Typed in once; never shown again.
...
4. **Battery:** tap "Don't battery-optimise this app", and in Samsung's Device care /
   Battery settings add SAiM Sensor to the apps that are never put to sleep.
...
is remembered by the SAiM frontend, so the screen asks
`/api/presence/display?room=study` and shows SAiM / clock / locked for the study.

## Build

GitHub Actions (`.github/workflows/android-sensor.yml`) runs the unit tests and builds
on every push to `saim/android-sensor/**`, then replaces `saim-sensor.apk` on the
`android-sensor-latest` pre-release. The APK contains no secrets.

Signing uses a stable key from two repo secrets, `SAIM_SENSOR_KEYSTORE_B64` and
`SAIM_SENSOR_KEYSTORE_PASSWORD`. A stable key is what lets an update install over the
