 ...
 * `POST /api/presence/sensor` — the same contract saim-room-sensor.py uses, so the
 ...
 *    loop exactly when SAiM is showing the locked screen: a partial wake lock and a Wi-Fi
...
class SensorService : Service() {
    ...
    private val thread = HandlerThread("saim-sensor").apply { start() }
    ...
    // SAiM's greeting on arrival (11 Sep 2026). NEURO chooses the words and whether to
    // speak; saim/backend hands them over in the reply to a reading; this only says
    ...
    private fun push(reading: Map<String, Any?>) {
        ...
        pushExecutor.execute {
            try {
                ...
                if (token.isNotEmpty()) conn.setRequestProperty("X-Saim-Sensor-Token", token)
    ...
    private fun acquireLocks() {
        if (wakeLock?.isHeld != true) {
            wakeLock = (getSystemService(Context.POWER_SERVICE) as PowerManager)
                .newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "saim:sensor").apply { acquire() }
        }
        if (wifiLock?.isHeld != true) {
            wifiLock = (applicationContext.getSystemService(Context.WIFI_SERVICE) as WifiManager)
                .createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "saim:sensor").apply { acquire() }
        }
    }
    ...
    private fun startInForeground() {
        ...
        val notification = Notification.Builder(this, CHANNEL)
            .setContentTitle("SAiM room sensor")
    ...
    companion object {
        private const val TAG = "SaimSensor"
