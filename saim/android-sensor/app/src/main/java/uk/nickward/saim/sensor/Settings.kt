 ...
 * never in a log line, never shown back on screen. Same rule as `/etc/saim-watch.env`.
 */
class Settings(context: Context) {
    private val prefs = context.getSharedPreferences(
        // ⚠ The PRE-RENAME store name, kept on purpose. This file holds the
        // provisioned NEURO token and room for this tablet. Rename it and the app
        // reads an empty store on next launch — it comes up unprovisioned and
        // reports nothing, which looks exactly like a sensor that has gone quiet.
        "sara_sensor", Context.MODE_PRIVATE)
