...
//
// ⚠ ONE source, shared by the phone (`saim/app` Surface) and the Pi kiosk
// (`saim/frontend` Presence). Nick's steer on 30 Aug was that the two "should
...
export default function AttentionSurface({
  ...
  // ⚠ 'approach' is SAiM's screen (Nick, 13 Sep 2026). `?look=list` is the way
  ...
  const {
    ...
    // by `saim-surface` so four renderers cannot word it four ways.
    silence = null,
    // ⚠ WHY THIS PANEL IS THE ONE ON SCREEN. `saim-surface` has composed it
    ...
    >
      ...
      <div className="surface__content">
        <div className="surface__crown">
          <span className="surface__mark">SAiM</span>
          <button
            type="button"
            className="surface__state"
            onClick={() => setShowWhy((v) => !v)}
            aria-expanded={showWhy}
            aria-label="Why SAiM is showing this"
          >
              ...
              {!transitionSaysPrimary && (
                <>
                  ...
                  {/* ⚠ He is ALREADY ON IT. The brain stamps `session` on the
                      ...
                      SAiM went on reading "you have not replied yet" as if
                ...
                <>
                  <div className="surface__acts">
                    ...
                    {/* ⚠ "Not now" opens the durations rather than deferring on
                        a guess. A snooze whose length SAiM picked is one Nick
                  ...
                  {deferring && (
                    <div className="surface__acts surface__acts--defer">
                      ...
                      {/* Seen is NOT a snooze. It stops SAiM asking again while
        ...
        {/* ── What she's showing ────────────────────────────────────────────
            Nick, 31 Aug 2026: SAiM is "a series of dashboards", and the
