#!/usr/bin/env bash
# SAiM Mission Control — Pi 5 desktop launcher (WS2-WP1).
#
# Opens the SAiM UI full-screen on the Pi 5 desktop. This is the *display* launcher
# (what the SAiM.desktop icon runs); the *runtime* is brought up separately and kept
...
SAIM_URL="${SAIM_URL:-http://localhost:3005/}"
SAIM_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

health_ok() {
  curl -fsS "${SAIM_URL}api/health" >/dev/null 2>&1
}

# Make sure the backend is actually answering before we open a window.
if ! health_ok; then
  echo "[saim] runtime not responding on ${SAIM_URL} — nudging PM2"
  if command -v pm2 >/dev/null 2>&1; then
    pm2 start "$SAIM_DIR/runtime/ecosystem.config.js" --update-env >/dev/null 2>&1 \
      || pm2 restart saim-backend --update-env >/dev/null 2>&1 \
      || true
  fi
...
if ! health_ok; then
  echo "[saim] backend still unreachable on ${SAIM_URL}." >&2
  echo "[saim] bring the runtime up first:  bash ${SAIM_DIR}/runtime/start.sh" >&2
  exit 1
fi

echo "[saim] opening Mission Control at ${SAIM_URL}"
...
# and simply never opens a window — i.e. "SAiM won't load from the desktop". Only
...
# --- Hide the Pi taskbar (wf-panel-pi) for the SAiM session ---
...
# stop the supervisor + panel for the duration of the SAiM session and restore
...
# cookie store in a plain local file instead, so there is no keyring prompt. SAiM holds
# no secrets in the browser, so there is nothing to protect here.
# --disk-cache-size=1 / --disable-application-cache: the SAiM frontend is redeployed
