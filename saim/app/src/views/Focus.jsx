...
// Renders the brain's /api/focus: an optional SAiM briefing, the recommended next
// action, then the prioritised items (tiered, scored, with a reason each).
const URGENCY = ['critical', 'high', 'medium', 'low'];
const { resolveSaimLitePlan } = actionSurfaces;
...
export default function Focus({ onNavigate, onActionIntent }) {
  ...
  // because SAiM's job is to come to Nick rather than wait to be opened.
  //
  // Gated on the same persisted `saim_voice_out` toggle as Chat (OFF by default —
  ...
  useEffect(() => {
    const briefing = state.data?.saim?.briefing;
  ...
  function presentIntent(intent, fallbackTab = 'focus') {
    ...
    onActionIntent?.({
      ...
      title: intent.title || intent.label || intent.reason || intent.kind || 'SAiM action',
      url: intent.url || null,
      payload: {
        ...intent.payload,
        ...intent,
      ...
      {data && (
        <>
          {/* ⚠ HER SENTENCE, composed server-side and rendered verbatim — so a
              ...
              actually said. */}
          {data.saim?.briefing && <Lit tone="statement" className="focus__briefing">{data.saim.briefing}</Lit>}
