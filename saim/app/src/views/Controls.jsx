...
// SAiM Controls — the surface where Nick decides how much SAiM is allowed to
...
//     surfaced. Without it "SAiM feels inconsistent" is unanswerable, which is
...
export default function Controls() {
  ...
  // Prompts SAiM has quietened. Loaded apart from the settings on purpose: a
  ...
  async function unmute(kind) {
    ...
    try {
      ...
      setMutedNote(`${MUTED_KIND_WORDS[kind] || kind} is back on. SAiM starts judging it afresh.`);
    } catch (e) {
  ...
  if (error && !settings) {
    return (
      <div className="controls">
        <h2 className="controls__title">SAiM Controls</h2>
    ...
    <div className="controls">
        ...
        <section className="controls__section">
          ...
          <p className="controls__muted">
            {permission === 'denied'
              ...
              : 'Off. SAiM can still be read here; she just will not interrupt you.'}
          </p>
          {permission !== 'denied' && (
      ...
      <section className="controls__section">
        <label className="controls__row">
          <span>
            <strong>Let SAiM interrupt me</strong>
      ...
      <section className="controls__section">
        <h3 className="controls__heading">
          Quietened by SAiM{Array.isArray(muted) ? ` (${muted.length})` : ''}
        </h3>
        ...
        {Array.isArray(muted) && muted.map((m) => (
          <div key={m.kind} className="controls__record">
            ...
            <p className="controls__stamps">
              {m.by === 'saim' ? 'SAiM quietened it' : `muted by ${m.by}`}
              {m.at ? ` ${when(m.at)}` : ''}
            </p>
            <button
      ...
      <section className="controls__section">
        <h3 className="controls__heading">On SAiM's mind ({records.length})</h3>
