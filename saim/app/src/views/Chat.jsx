...
import { speakSaim, isVoiceOutEnabled, setVoiceOutEnabled, unlockAudio } from '../voiceUtils';
import './Chat.css';
...
const VOICE_KEY = 'saim_tts_voice';
// Pre-rename key. A lost voice choice falls back to the system default — not
// broken, but it is a preference Nick set and nothing would say why it moved.
const LEGACY_VOICE_KEY = 'sara_tts_voice';
const SAMPLE_LINE = 'Queue is at twelve, three at risk. Chase Abdi today, or hand it to Stephen.';

function getTtsVoice() {
  try { return localStorage.getItem(VOICE_KEY) || localStorage.getItem(LEGACY_VOICE_KEY) || 'coral'; } catch { return 'coral'; }
}

const VOICE_ERRORS = {
  'not-allowed': 'Microphone blocked. Allow it in Settings → Safari → Microphone, then reload.',
  'service-not-allowed': 'iOS refused speech recognition here. Try opening SAiM in Safari rather than the installed app.',
...
// What SAiM reached for during a reply, in plain words.
//
...
export default function Chat() {
  ...
  // Speak SAiM's reply once it has finished arriving.
  ...
  function say(text) {
    const utterance = speakSaim(text);
    ...
    utterance.onerror = (evt) => {
      ...
      console.warn('[SAiM Voice] utterance error', evt?.error, evt);
    };
  ...
  async function speakViaServer(text, voiceOverride) {
    ...
    try {
      ...
      console.warn('[SAiM Voice] server TTS failed', err);
    } finally {
  ...
  function startVoice() {
    ...
    rec.onerror = (evt) => {
      ...
      console.warn('[SAiM Voice] recognition error', code, evt);
    ...
    try {
      ...
      console.warn('[SAiM Voice] start() threw', err);
