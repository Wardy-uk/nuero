...
// SAiM mobile app shell.
//
// SAiM is the J.A.R.V.I.S. layer — voice, ears and eyes — and should NOT be a
...
//
// Neuro Mobile is the app Nick opens; SAiM is the layer that comes to him. So
...
// ⚠ THE SCREEN SET MOVED to `saim/shared-ui/tabs.jsx` (31 Aug 2026), where the
// Pi kiosk mounts the SAME list. Nick: "make the Pi version of SAiM the same as
...
const { resolveSaimLitePlan, resolveSaimLiteTab } = actionSurfaces;
...
function readLaunchIntent() {
  ...
  return {
    ...
    tab: VALID_TABS.has(tab) ? tab : resolveSaimLiteTab({ tab, type: type || raw.type, url: url || raw.url, payload: raw }),
...
export default function App() {
  ...
  // ⚠ SAiM is the default screen, always (Nick, 30 Aug 2026). Phase 2 opened on
  // `Now`, and that was wrong: opening on a list makes this an app you read,
  // when the whole point is that SAiM is PRESENT and comes to him. Her field —
  ...
  // What SAiM said when she pinged him, carried onto the Surface so tapping a
  ...
  useEffect(() => {
    ...
    if (!intent) return;
    const plan = resolveSaimLitePlan(intent);
  ...
  useEffect(() => {
    ...
    function onMessage(event) {
      ...
      if (data?.type !== 'saim-notification-open') return;
      const tab = VALID_TABS.has(data.tab) ? data.tab : resolveSaimLiteTab(data);
      const intent = {
        source: 'notification',
        tab,
  ...
  // is NOT in PRIMARY, so tapping SAiM left the whole secondary strip open
  ...
  return (
    <div className="app lit-scope" style={{ '--saim-rgb': surfaceRgb(fieldDrive) }}>
      {/* ⚠ SAiM's substrate is present on EVERY screen (Nick, 31 Aug 2026), not
      ...
      <header className="app__header">
        ...
        <button
          ...
          type="button"
          onClick={refreshApp}
          aria-label="Refresh SAiM mobile"
          title="Refresh SAiM mobile"
          disabled={refreshing}
        >{refreshing ? '…' : '↻'}</button>
        {wakeLock.supported && (
          <button
