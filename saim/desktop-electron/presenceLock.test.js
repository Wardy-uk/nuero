...
test('present → away x2 → warn → grace elapses → lock', () => {
  ...
  assert.equal(state.lockedBySaim, true);
});
...
test('⚠ fresh keyboard/mouse input cancels a countdown, even if the Watch reads away', () => {
  ...
  assert.equal(state.lockedBySaim, false);
});
...
test('coming back to a machine SAiM locked wakes the display, once, and re-arms', () => {
  const locked = run([at(0, 'present'), at(5000, 'away'), at(10000, 'away'), at(15000, 'away')]);
  assert.equal(locked.state.lockedBySaim, true);
...
test('while SAiM holds the lock, more "away" does nothing — one lock, not a loop', () => {
  const locked = run([at(0, 'present'), at(5000, 'away'), at(10000, 'away'), at(15000, 'away')]);
  const more = run([at(20000, 'away'), at(25000, 'away'), at(40000, 'away')], locked.state);
  assert.deepEqual(more.actions, [null, null, null]);
});

test('an unlock by any other means (Hello, PIN) releases SAiM\'s claim — no surprise wake', () => {
  const locked = run([at(0, 'present'), at(5000, 'away'), at(10000, 'away'), at(15000, 'away')]);
