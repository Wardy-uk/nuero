...
// build (saim.nickward.co.uk, 1 Sep), which never had the hook, and the kiosk
...
// intact. Deciding it here makes it independent of WHICH SAiM the window loads.
//
...
function step(prev, { payload, now, idleS }) {
  ...
  if (!r.away) {
    ...
    s.armed = true;
    if (s.lockedBySaim) {
      // He came back to a machine SAiM locked: nudge the display so Hello can sign
      // him in. Hello does the authentication — nothing here unlocks anything.
      s.lockedBySaim = false;
  ...
  // Away.
  if (s.lockedBySaim || !s.armed) return { state: s, action: null };
...
/** Windows unlocked by any means: SAiM no longer holds the lock. */
function onOSUnlocked(prev) {
  return { ...prev, lockedBySaim: false, graceUntil: null, awayCount: 0 };
}
