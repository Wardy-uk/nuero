...
// usePresenceLock — SAiM's "walked away" auto-lock (WS2-WP3).
//
...
export function usePresenceLock({
  ...
  // (window.saimNative). Undefined in a plain browser and disabled on the Pi kiosk
  ...
  useEffect(() => {
    let alive = true;
    window.saimNative?.capabilities?.()
  ...
  const lock = useCallback((why) => {
    ...
    if (osCaps.current?.osLock) window.saimNative?.lockOS?.();
  }, []);
  ...
  const cancelCountdown = useCallback(() => {
    ...
    setPending(null);
    window.saimNative?.attention?.(false); // release the front-and-centre warning window
  }, []);

  // Grace countdown before an AWAY lock: instead of locking instantly we show a visible
  // "Locking…" countdown and bring SAiM to the front. Any activity — your keyboard/mouse
  ...
  const startCountdown = useCallback((why) => {
    ...
    setPending(secs);
    window.saimNative?.attention?.(true); // pop to front so the warning is seen anywhere
    if (countdownTimer.current) clearInterval(countdownTimer.current);
  ...
  const autoUnlock = useCallback(() => {
    ...
    if (osCaps.current?.osLock) window.saimNative?.wakeOS?.();
    resetIdle();
  }, [resetIdle]);
  ...
  // Hello, the Electron shell emits 'os-unlocked' — lift SAiM's overlay too, since the
  // OS already re-authenticated you. No-op in a plain browser / on the Pi (no saimNative).
  useEffect(() => {
    const off = window.saimNative?.onOSUnlock?.(() => unlock());
