...
import { DEFAULT_VIEW, normalizeViewId, SAIM_VIEWS } from './views';

// SAiM shared state/context — the single in-app source of truth for every screen
// (WS2-WP1).
//
...
const SaimStateContext = createContext(null);
...
function deriveOpeningMessages(model) {
  if (!model) return [];
  const messages = [
    createMessage('saim', `Hi Nick. You're at ${model.location?.label || 'an unknown spot'}.`, { kind: 'opening' }),
  ];
  if (model.briefing?.line) {
    messages.push(createMessage('saim', model.briefing.line, { kind: 'briefing' }));
  }
  return messages;
}
...
function mapFocusActionTargetToView(target) {
  ...
  if (normalized === 'queue') return SAIM_VIEWS.QUEUE;
  if (normalized === 'todos') return SAIM_VIEWS.TODOS;
  if (normalized === 'standup' || normalized === 'meeting-prep') return SAIM_VIEWS.STANDUP;
  if (normalized === 'imports' || normalized === 'brain') return SAIM_VIEWS.VAULT;
  if (normalized === 'capture') return SAIM_VIEWS.CAPTURE;
  if (normalized === 'chat') return SAIM_VIEWS.SAiM;
  if (normalized === 'inbox') return SAIM_VIEWS.QUEUE;
  return SAIM_VIEWS.FOCUS;
}
...
// desktop Notification ("SAiM needs your eyes") off this file's own `/api/state`
...
// and navigated to legacy view ids no screen mounts. SAiM interrupting belongs to
// NEURO's governor and the attention lifecycle, not to a poll in the kiosk.

export function SaimStateProvider({ children }) {
  ...
  function applyIncomingModel(data) {
    ...
    if (
      ...
      setInterruptionNotice({
        ...
        detail: nextUrgentSnapshot.top.detail || 'SAiM detected a new urgent change.',
  ...
  function openFocusAction(action) {
    if (!action) {
      navigateToView(SAIM_VIEWS.FOCUS);
  ...
  useEffect(() => {
    const releaseAttention = () => {
      if (!isWindowBackgrounded()) {
        window.saimNative?.attention?.(false);
      }
    };
  ...
  async function sendChat(message) {
    ...
    const placeholderId = `saim-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    setChatError(null);
    setChatStatus('sending');
    setChatMessages((current) => [
      ...current,
      createMessage('user', trimmed),
      createMessage('saim', '', { id: placeholderId, pending: true }),
    ]);
      ...
      setChatMessages((current) =>
        current.map((entry) =>
          entry.id === placeholderId && !entry.text
            ? { ...entry, text: 'SAiM answered, but the upstream stream carried no readable text.', pending: false }
            ...
            ? {
                ...
                text:
                  nextStatus === 'unavailable'
                    ...
                    : `SAiM could not reach the NEURO chat upstream — ${e.message}`,
                error: true,
              }
            : entry
        )
  ...
  // it. The SAiM backend answers `{ ok, saved, error }` and `saved` is the field that
  ...
  async function submitCapture(path, payload) {
    ...
    try {
      ...
      // The SAiM backend itself is unreachable — the phone/kiosk is offline, or the
      // process is down. Not saved, and there is no queue to fall back on by design.
      return { ok: false, saved: false, error: `SAiM is offline — the capture was NOT saved (${e.message}).` };
    }
  ...
  // the connection banner reads exactly this. When the SAiM backend itself has not
  ...
  const provenance = model?.provenance || {
    ...
    message:
      status === 'disconnected'
        ? `SAiM's own backend is unreachable${error ? ` — ${error}` : ''}. Nothing on screen is current.`
        : 'Connecting to SAiM…',
    neuro: null,
  };
  ...
  return <SaimStateContext.Provider value={value}>{children}</SaimStateContext.Provider>;
}
...
export function useSaimState() {
  const ctx = useContext(SaimStateContext);
  if (!ctx) throw new Error('useSaimState must be used within a SaimStateProvider');
  return ctx;
}
