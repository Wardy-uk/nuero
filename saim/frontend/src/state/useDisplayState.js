...
//    is watch here → SAiM, watch elsewhere → clock, out of the house → off.
//
...
// unrecognised state — leaves the screen showing SAiM. A bug that fails to
...
// to the build-time VITE_SAIM_ROOM, then the living room, so the existing kiosk is
...
function resolveRoom() {
  ...
  try {
    if (fromUrl) { window.localStorage.setItem('saim_room', fromUrl); return fromUrl; }
    const stored = clean(window.localStorage.getItem('saim_room'));
    if (stored) return stored;
  } catch { /* storage unavailable */ }
  return fromUrl || import.meta.env.VITE_SAIM_ROOM || 'living-room';
}
