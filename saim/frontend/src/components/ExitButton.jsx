...
// (confirm-then-quit) so a stray touch can't drop you out: tap shows "Close SAiM?",
// and only "Close" actually quits. On confirm it asks the SAiM backend to close the
...
// a stray palm must not close SAiM. Cancel is the wider of the two, because
...
export default function ExitButton({ variant = 'chrome' }) {
  ...
  async function quit() {
    ...
    try {
      ...
      // that never closes read as SAiM hanging.
      if (body.ok === false) {
        setCannot(body.error || 'SAiM can’t close this screen from here.');
        setPhase('cannot');
        return;
      }
    } catch {
  ...
  if (variant === 'nav') {
    return (
      <div className="exitnav">
        ...
        {phase === 'confirm' && (
          <div className="exitnav__confirm" role="dialog" aria-label="Close SAiM?">
            <span className="exitnav__ask">Close SAiM?</span>
            <div className="exitnav__acts">
              <button type="button" className="exitnav__act exitnav__act--no" onClick={() => setPhase('idle')}>
                Cancel
              </button>
  ...
  if (phase === 'confirm') {
    return (
      <div className="exit exit--confirm" role="dialog" aria-label="Close SAiM?">
        <span className="exit__ask">Close SAiM?</span>
