...
// SAiM on the Pi — the same app as the phone.
//
// ⚠ WHAT THIS REPLACED, and why. Nick, 31 Aug 2026: "make the Pi version of
// SAiM the same as phone app - same for the desktop." It was not. The kiosk had
// its own fourteen-screen registry rendering `saim/backend`'s state model,
...
// (`saim/shared-ui/tabs.jsx`) and both shells mount it.
//
// The DESKTOP comes free: `saim/desktop-electron` loads this same frontend from
// `SAIM_URL`, so it is the same app by construction rather than by a third port.
//
...
// same-origin; `saim/backend` answers its own named routes and forwards the
...
// origin as the SPA, so `saim/backend` answers and attaches the credential.
...
function AppShell() {
  ...
  // ⚠ Driven on every screen. The kiosk reads through `saim/backend`'s
  ...
  // ⚠⚠ HER COLOUR, SET ONCE, FOR THE WHOLE SHELL — see `saim/app/src/App.jsx`,
      ...
      <FieldCover covered={overlayOwnsScreen}>
        ...
        <nav className="app__nav" aria-label="SAiM">
          ...
          {/* ⚠ In the nav rather than the top bar (Nick, 31 Aug 2026), and LAST —
              ...
              kept: a stray palm must not close SAiM. */}
