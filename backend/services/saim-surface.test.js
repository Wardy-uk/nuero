 ...
 *
 *   run: node --test backend/services/saim-surface.test.js
 */
...
test('⚠ an unreadable pool outranks EVERY activity', () => {
  // A dashboard drawn over work SAiM could not see is a confident picture of a
...
test('"That’s finished" leads in a meeting, and is a `meeting` intent', () => {
  ...
  // minutes early otherwise costs twenty minutes in which SAiM refuses to help.
...
test('⚠ "finished" is offered ONLY in a meeting', () => {
  ...
  // decides that, and a manual way to make SAiM go quiet is a mute button
...
test('⚠ "not now" carries HOW LONG and WHY', () => {
  // A snooze whose length SAiM picked is one Nick has no reason to trust, and a
...
test('⚠ no habit yet means NO line — not "about normal"', () => {
  ...
  // a guess and the register saim-voice rejects.
