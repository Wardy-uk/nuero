...
/**
 * ONE VOCABULARY, ACROSS EVERY SAiM SURFACE — the guard the per-screen passes
 * needed and did not have.
 *
 * ⚠⚠ THE BUILD ORDER FIXED TEN SCREENS AND THE PALETTE WAS STILL FRAGMENTED.
 * A sweep after the last one counted, in `saim/app/src` alone, **eight distinct
...
const DIRS = [
  path.join(ROOT, 'saim', 'app', 'src'),
  path.join(ROOT, 'saim', 'shared-ui'),
];

/** Every colour SAiM is allowed to use, and what each one MEANS. */
const VOCABULARY = new Map(Object.entries({
  // ── Her ────────────────────────────────────────────────────────────────
  // Her light is never a literal: it comes from `--saim-rgb`, which the shell
...
test('⚠⚠ every colour SAiM uses is one she has a word for', () => {
...
test('⚠ her light is never a literal', () => {
  // Every screen takes her colour from `--saim-rgb`, set once on the shell from
