 ...
 *
 * ⚠ SARA-ONLY SCREENS TAKE `Palette.sara.*`, THE STATIC. They are never hosted
...
function swiftFiles() {
  ...
  ['Sara', 'SaraWidgets', 'SaraWatchComplication',
...
test('⚠ a running session is a fact, not a warning', () => {
  if (!present()) return;
  const src = fs.readFileSync(path.join(IOS, 'Sara', 'AttentionSurfaceView.swift'), 'utf8');
...
test('⚠ a countdown is a duration, and its row carries the urgency', () => {
  if (!present()) return;
  const src = strip(fs.readFileSync(path.join(IOS, 'Sara', 'DashboardView.swift'), 'utf8'));
  assert.match(src, /row\.countdown/, 'could not read DashboardView');   // positive control
  assert.ok(!/foregroundStyle\(Palette\.sara\.gap\)[\s\S]{0,80}row\.when/.test(src),
...
test('⚠ a view that renders in BOTH apps asks its host', () => {
  ...
  // `Palette.sara.*` inside NeuroKit would put her material on NEURO's desk.
  const shared = path.join(IOS, 'NeuroKit', 'Sources', 'NeuroKit');
  const offenders = [];
  for (const file of swiftFiles().filter((f) => f.startsWith(shared))) {
    const body = strip(fs.readFileSync(file, 'utf8'));
    if (/Palette\.sara\b|Palette\.neuro\b/.test(body)) offenders.push(path.basename(file));
  }
  assert.deepEqual(offenders, [],
...
test('⚠ a widget takes the static, because it cannot read an environment', () => {
  ...
  const w = fs.readFileSync(path.join(IOS, 'SaraWidgets', 'SaraWidgets.swift'), 'utf8');
  assert.match(w, /Palette\.sara\.gap/,
