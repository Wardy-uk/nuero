...
/**
 * What SAiM SHOWS, and what Nick could SAY next.
 *
 * ⚠ WHY THIS EXISTS. Nick, 31 Aug 2026, on what SAiM actually is:
 *
 *   "if we start with the principle that SAiM is a manifestation so shouldn't be
 ...
 * decided. That boundary is not stylistic: `saim/backend/src/state/inference.js`
 ...
 * drawn over work SAiM could not see is a confident picture of a day nobody
...
/**
 * What could not be read, in SAiM's own words where she has them.
 *
...
function dashRitual(payload, now) {
  ...
  if (which === 'standup') {
    ...
    //   register saim-voice rejects - so the row is simply absent.
 ...
 *   named zone; `room` is SAiM's own BLE classification. The room is the more
 ...
 *
 * ⚠ PURE, and it takes the clock rather than reading one — `saim-surface` has
...
function dashOffDuty(payload, now) {
  ...
  //   and inventing one is the register saim-voice rejects.
  const did = payload.didRecently;
 ...
 * SAiM does not put work in front of him while he is in a room with people, and
...
function utterancesFor(payload, surface, session) {
  ...
  if (surface === SURFACES.IN_MEETING) {
    ...
    // which SAiM refuses to help — off the calendar's word alone, with no way
  ...
  if (actionable) {
    ...
    // ⚠ "Not now" carries HOW LONG and WHY. A snooze whose length SAiM picked is
