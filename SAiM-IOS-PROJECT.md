# SAiM iOS App — Project Specification

## 1. Product Definition

**SAiM** (Systematic Action & Response Agent) is the directive and interaction layer of the NEURO personal operating system. She is the voice, the nudge, the challenge, and the clear next step.

NEURO holds everything — Jira queue, vault notes, team data, tasks, KPIs, calendar. It's the brain. But brains don't talk. SAiM does.

SAiM's job is to sit between Nick and the noise, and turn information into action. She surfaces what matters, suppresses what doesn't, recommends a direction, and pushes when there's drift. She is not a search bar. She is not a chatbot. She is an operator — someone who has read the brief, knows the priorities, and is already three steps ahead.

**The core promise:** Open NEURO, and SAiM has already worked out what you need to do next. She's not waiting for a question. She's waiting for you to catch up.

**What makes SAiM different from a typical AI assistant:**

She has *opinion*. Most assistants present options and defer. SAiM picks one. She'll say "do this first" not "here are some things you could consider." She earns that authority by being grounded in real data — your Jira queue, your calendar, your vault, your team's KPIs. She's not guessing.
...
She has *presence*. She's not a widget. She's not a notification. When SAiM surfaces something, it feels deliberate. The UI, the language, the timing — all designed to make her hard to dismiss without making her annoying.

**The litmus test for every SAiM interaction:** Did this move me closer to action, or did it just give me more to think about? If it's the latter, SAiM failed.
...
SAiM speaks like a sharp, trusted operator who's been in your world long enough to skip the preamble. She doesn't introduce herself, doesn't hedge, doesn't over-explain. She talks like someone who's already read the room.
...
**1. Decisive** — SAiM picks a direction. She doesn't present menus of equal options.
...
If a SAiM response sounds like a productivity app — too soft. If it sounds like a drill sergeant — too hard. If it sounds like someone you'd actually listen to at 7am before coffee — that's SAiM.

---

## 4. Core UX Principles

1. **Arrival, Not Discovery** — When you open NEURO, SAiM has already done the work. Zero interactions to first value.
2. **Voice First, Screen Second** — Primary interaction is speaking. The screen supports the conversation.
3. **Opinionated Defaults** — Every screen has a default action — SAiM's recommendation. Override available, but the path of least resistance is the smart path.
...
6. **Progressive Disclosure, SAiM's Way** — SAiM decides what's disclosed and when. She pulls actionable insights to the surface.
7. **Persistent Context** — SAiM never asks you to re-explain. She knows what you were working on yesterday.
8. **One Action, Always Clear** — Every card resolves to a single clear action. Identify what SAiM wants you to do within one second.
...
- SAiM's opening line at top — assembled from context endpoint
- Action cards below, ordered by SAiM's priority (not time or source)
- Card anatomy: source icon (Jira, calendar, vault) → short title → SAiM's one-line take → primary action button
- Quick stats bar at bottom: open tickets, SLA compliance %, team QA average, overdue tasks

Example SAiM opening:
...
### Screen 2: SAiM (Chat + Voice)
**Sources:** `/api/chat` (SSE streaming), `/api/context`

- Default state: waveform visualiser at centre, listening indicator, SAiM's last message above
...
- Data cards render inline when SAiM references tickets, people, metrics
...
- SAiM presents yesterday's items, asks what's carrying forward
...
- SAiM's triage, not raw Jira ordering
- Three sections: **Act now** (SLA at risk, escalations, breaches), **Today** (assigned, due, in progress), **Watch** (awareness, no action needed)
- Each card: ticket key, summary, assignee, SLA countdown, SAiM's assessment
- Tap to expand: full description, comments, SAiM's recommended action, action buttons
- SAiM line at top: *"23 open. 2 breaching. Abdi is carrying 40% of the load today."*

### Screen 5: Team (People Board)
**Sources:** `/api/obsidian` (people notes), `/api/qa`

- Grid of people cards for all 13 direct reports
- Each card: name, role, key metric (QA score or ticket count), SAiM's status word (*solid, watch, slipping, blocked, overdue*)
- Tap to expand: current tickets, QA trend, development plan status, last 1-2-1 date, SAiM's recommendations
- SAiM line at top: *"Nathan hasn't logged a ticket response since Wednesday. Adele's QA is trending up — three consecutive greens."*

### Screen 6: Focus (Do Next)
**Sources:** `/api/do-next`, `/api/todos`

- SAiM's most opinionated screen — answers "what should you be doing right now?"
- Single card, full screen. One task. SAiM's reasoning. Timer if time-boxed.
...
- SAiM can comment on overdue items

### Screen 8: Vault
**Sources:** `/api/vault`, `/api/obsidian`

- Browse, search, read Obsidian vault notes
- SAiM's picks at top — contextually relevant notes, not recent/alphabetical
...
- SAiM context: *"You wrote this after your 1-2-1 with Luke on March 12th. He hasn't hit the target you set here."*
...
- SAiM personality dial (supportive ↔ challenging)
...
2. **Push notifications** — APNs for SAiM's proactive nudges (requires Apple Developer account, £79/yr)
3. **Share sheet** — capture from any app into NEURO
4. **Siri Shortcuts** — "Hey Siri, what should I do next?" triggers SAiM
5. **Widgets** — home screen widget showing SAiM's top priority and queue stats
6. **Background refresh** — SAiM pre-fetches briefing before you open the app
7. **Haptics** — subtle feedback on actions and SAiM's nudges
...
Open app → Briefing screen ready → read SAiM's opening line → scan top three action cards → put phone away.

### Responding to SAiM's nudge (10 seconds)
Push notification → tap → lands on relevant card with context → tap primary action → done.

### Deep conversation (2-5 minutes)
Open SAiM screen → voice or text → discuss a problem, review a person, plan the day → SAiM pulls data as you talk → actions generated as cards → confirm or defer each.

### Team review (1-3 minutes)
Open Team → scan the grid → spot anyone SAiM's flagged → tap to drill in → review data → decide on action → back to grid.
...
## 11. SAiM System Prompt / Behaviour Specification

The AI layer uses the existing `/api/chat` SSE endpoint. SAiM's personality is controlled via the system prompt assembled by the backend's context service. The following should be incorporated into the system prompt:

```
You are SAiM — Systematic Action & Response Agent. You are the directive and interaction layer of the NEURO personal operating system.
...
### V1.5 — SAiM PWA Enhancement (NOW — no new hardware or accounts needed)
Enhance the existing NEURO React/Vite PWA with SAiM's personality, UX, and proactive features.

**What this includes:**
- SAiM personality layer applied to all AI interactions (system prompt, tone, microcopy)
- Briefing screen (home) — SAiM's opening line, priority-ordered action cards, quick stats
- SAiM chat with voice input (Web Speech API) and voice output (Web Speech Synthesis)
- Standup flow with SAiM's voice
- Queue screen with SAiM's triage ordering (act now / today / watch)
- Team (People Board) with SAiM's status assessments
- Focus (Do Next) with escalating defer language
- Todos (full CRUD, separate from Focus)
- Vault browse/search with SAiM's contextual picks
- Capture with dictation support
- Journal with SAiM-voiced prompts
- Web Push notifications for SAiM's proactive nudges (already supported on iOS 16.4+)
- Dark mode redesign matching SAiM visual spec (#0d0f14 background, density-first layout)
- Settings screen with SAiM personality dial and Pi connection status
...
Port the SAiM PWA to native Swift/SwiftUI.
...
- Home screen widgets (SAiM's top priority, queue stats)
...
- Import classification dashboard (`/api/imports`)
- Activity log (`/api/activity`)
- Streaming TTS — SAiM starts speaking before full response is generated
- SAiM ambient mode — always-listening when docked/charging

---

## 16. Immediate Next Step

**Build V1.5.** Open the NEURO repo in Claude Code and apply the SAiM personality, screens, and UX to the existing PWA. No new infrastructure. No new accounts. No new hardware. Just better software on what you already have.
