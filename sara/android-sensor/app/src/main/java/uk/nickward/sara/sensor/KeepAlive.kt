...
object KeepAlive {
    ...
    fun schedule(context: Context) {
        ...
        Log.i("SaraSensor", "keep-alive job ${if (result == JobScheduler.RESULT_SUCCESS) "scheduled" else "REFUSED"}")
    }
...
class KeepAliveJob : JobService() {
    override fun onStartJob(params: JobParameters?): Boolean {
        if (Settings(this).enabled && !SensorService.running) {
            Log.w("SaraSensor", "keep-alive: the sensor was not running — starting it")
