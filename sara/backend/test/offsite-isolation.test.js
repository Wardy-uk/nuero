...
// screen showed SARA to an empty room, with the contradiction line explaining that it
...
function withOffsite(rooms, fn) {
  const before = process.env.SARA_OFFSITE_ROOMS;
  process.env.SARA_OFFSITE_ROOMS = rooms;
  try { return fn(); } finally {
    if (before === undefined) delete process.env.SARA_OFFSITE_ROOMS; else process.env.SARA_OFFSITE_ROOMS = before;
  }
}
