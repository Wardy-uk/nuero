#!/usr/bin/env python3
# SAiM terminal agent — turns this Pi into a presence-aware SAiM terminal.
#
...
#        * launch the SAiM kiosk if it isn't running
#        * report station present=true to SAiM   (POST /api/location/station)
#
#   -> AWAY (you left):
#        * report station present=false to SAiM
#        * blank the display           (wlopm --off)
#        * fallback: if blanking fails, lock SAiM in-app (best-effort)
#
# Stage 1: SAiM backend is this same Pi. Stage 2 (central NEURO server, many terminals):
# only SAIM_URL changes — the agent already speaks to SAiM purely over HTTP, exactly as a
...
SAIM_URL = os.environ.get("SAIM_URL", "http://localhost:3005").rstrip("/")
...
KIOSK_LAUNCHER = os.environ.get("KIOSK_LAUNCHER", "/mnt/data/nuero/saim/scripts/start-saim.sh")
...
def on_present():
    # Station reporting ONLY. The lock is the in-app SAiM overlay (driven by
...
def main():
    log(f"station={STATION_NAME} saim={SAIM_URL} presence_file={PRESENCE_FILE} "
